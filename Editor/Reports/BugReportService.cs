...
namespace DSGarage.FBX4VRM.Editor.Reports
{
    ...
    {
        // デフォルトサーバーURL - HTTPS ポート8443
        // URL: https://153.126.176.139:8443/bug-reports/submit
        // 更新日: 2024-12-01 - ポート8000から8443に変更
        private static readonly string DefaultServerUrlEncoded = "aHR0cHM6Ly8xNTMuMTI2LjE3Ni4xMzk6ODQ0My9idWctcmVwb3J0cy9zdWJtaXQ=";
        ...
        private static IEnumerator SendReportCoroutine(BugReportData report, string serverUrl, Action<bool, string> onComplete)
        {
            ...
            using (var request = new UnityWebRequest(serverUrl, "POST"))
            {
                ...
                {
                    Debug.Log($"[BugReportService] Report sent successfully: {report.ReportId}");
